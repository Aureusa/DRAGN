        ...
        for i in range(sources_arr.shape[0]):
            ...
            for i in range(num_fagn):
                ...
                if targets is not None:
                    ...
                    ax[i, 1].imshow(
                        targets[i, 0],
                        norm=ImageNormalize(targets[i, 0], interval=interval, stretch=AsinhStretch()),#norms[i],
                        cmap="gray"
                    )
                    ax[i, 1].axis("off")
                ...
                for m in range(num_models):
                    ax[i, 2 + m if targets is not None else 1 + m].imshow(
                        outputs[i, m],
                        norm=ImageNormalize(outputs[i, m], interval=interval, stretch=AsinhStretch()),#norms[i],
                        cmap="gray"
                    )
            ...
            plt.tight_layout()
            if save:
                filepath = os.path.join(data_folder, f"{filename}_{num_batch}.png")
                plt.savefig(filepath, bbox_inches="tight", dpi=200)
                print_box(f"Grid plot '{filename}_{num_batch}.png' saved successfully in {data_folder}!")
            else:
                plt.show()
            plt.close()
